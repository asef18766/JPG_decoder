...
struct _SUBVECTOR_{
    ...
    unsigned char quant;
}__attribute__((packed)) subVector[4];

unsigned char maxWidth, maxHeight;

typedef struct _acCode_ {
    unsigned char len;
    unsigned char zeros;
    int value;
}__attribute__((packed)) acCode;
...
pair* createHuffCode(unsigned char *a, unsigned int number) {
    int si = sizeof(pair);
    pair* ret = malloc(si * number);
    int code = 0;
    int count = 0;
    for (int i = 0; i < 16; i++) {
        for (int j = 0; j < a[i]; j++) {
...
//    printf("read AC: %d %d %d\n", codeLen, zeros, code);
    acCode ret={codeLen, zeros, code};
    return ret;
}

typedef struct _MCU_
{
    BLOCK mcu[4][2][2];
...
void quantify(MCU *this) {
    for (int id = 1; id <= 3; id++) {
        for (int h = 0; h < subVector[id].height; h++) {
            for (int w = 0; w < subVector[id].width; w++) {
                for (int i = 0; i < 8; i++) {
                    for (int j = 0; j < 8; j++) {
                        this->mcu[id][h][w][i][j] *= quantTable[subVector[id].quant][i*8 + j];
                    }
                }
            }
        }
    ...
    for (int id = 1; id <= 3; id++) {
        for (int h = 0; h < subVector[id].height; h++) {
            for (int w = 0; w < subVector[id].width; w++) {
                ...
                for (int i = 0; i < 8; i++) {
                    for (int j = 0; j < 8; j++) {
                        zz[i][j] = this->mcu[id][h][w][zz[i][j] / 8][zz[i][j] % 8];
                    }
                }
                for (int i = 0; i < 8; i++) {
                    for (int j = 0; j < 8; j++) {
                        this->mcu[id][h][w][i][j] = zz[i][j];
                    }
                }
            }
        }
...
//                     }
                ...
                for (int j = 0; j < 8; j++) {
                    for (int x = 0; x < 8; x++) {
                        for (int y = 0; y < 8; y++) {
                            s[j][x] += c (y) * this->mcu[id][h][w][x][y] * cos_cache[(j + j + 1) * y];
                        }
                        s[j][x] = s[j][x] / 2.0;
                    }
                }
